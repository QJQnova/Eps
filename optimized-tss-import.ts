...
async function optimizedTSSImport() {
  console.log('🚀 Начинаем ОПТИМИЗИРОВАННЫЙ импорт ВСЕХ TSS.RU товаров...');
    ...
    for (let i = 0; i < validProducts.length; i += batchSize) {
      ...
      const query = `
        INSERT INTO products (
          name, sku, slug, description, "shortDescription", price, 
          "originalPrice", "imageUrl", "categoryId", stock, "isActive", "isFeatured", tag
        ) VALUES ${values}
      `;
...
// Запускаем оптимизированный импорт
optimizedTSSImport()
  .then(() => {
    console.log('🏁 Оптимизированный импорт TSS завершен');
    process.exit(0);
  })
  .catch((error) => {
