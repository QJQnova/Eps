...
ИЗВЛЕКАЕМЫЕ ПОЛЯ (все 5 обязательны):
- name: Название товара на русском языке
- sku: Артикул (ОБЯЗАТЕЛЬНО используй оригинальный артикул/код из данных поставщика)
...
- description: Полное описание с характеристиками и техническими данными
- imageUrl: Всегда оставляй пустым ""

ПРАВИЛА:
1. НАЗВАНИЕ: Переведи на русский, сделай понятным для покупателей
2. АРТИКУЛ: Найди и используй оригинальный артикул/код из каталога поставщика
3. КАТЕГОРИЯ: Анализируй название и выбери точную категорию
4. ОПИСАНИЕ: Объедини все технические данные, характеристики, назначение
5. БЕЗ ЦЕН: Цены не обрабатываем
6. БЕЗ ИЗОБРАЖЕНИЙ: imageUrl всегда пустой
    ...
    if (rawProducts.length > adaptedProducts.length) {
      ...
      for (const rawProduct of remainingProducts) {
        const adaptedProduct: AdaptedProduct = {
          ...
          sku: rawProduct.sku || rawProduct.code || rawProduct.artikel || `AUTO-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
