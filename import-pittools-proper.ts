...
async function importPittoolsCSV() {
  console.log('Начинаю импорт каталога pittools.ru с правильным парсингом...');
  
  try {
    const csvContent = readFileSync('./pittools-import.csv', 'utf-8');
    
    // Находим строки, начинающиеся с URL изображений - это начало записей о товарах
    const productLines = csvContent.split('\n').filter(line => 
      line.startsWith('https://pittools.ru/upload/') && line.includes('.png;')
    );
    ...
    for (let i = 0; i < Math.min(productLines.length, 200); i++) {
      ...
      try {
        ...
        if (!categoryId) {
          try {
            ...
            console.log(`Ошибка с категорией "${categoryName}": ${error.message}`);
            errorCount++;
            continue;
          }
        }
