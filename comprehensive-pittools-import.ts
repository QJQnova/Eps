...
async function importPittoolsCSV() {
  ...
  try {
    ...
    // Ищем все строки с товарами - разные форматы изображений
    ...
    // Получаем существующие товары для исключения дубликатов
    try {
      const existingProducts = await storage.getAllProducts();
      const existingSKUs = new Set(existingProducts.products ? existingProducts.products.map(p => p.sku) : []);
    } catch (error) {
      console.log('Не удалось получить существующие товары, продолжаем импорт...');
    ...
    for (let i = 0; i < productLines.length; i++) {
      ...
      try {
        ...
        // Пропускаем если уже обработан или существует
        if (!name || !sku || !categoryName || !priceStr || 
            processedSKUs.has(sku) || existingSKUs.has(sku)) {
          skippedCount++;
          continue;
        }
