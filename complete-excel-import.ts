...
function readFileWithProperEncoding(filePath: string): string {
  try {
    ...
    try {
      // Если не получилось, пытаемся с windows-1251
      const buffer = readFileSync(filePath);
      return iconv.decode(buffer, 'windows-1251');
    } catch (error2) {
      try {
        // Если и это не сработало, пытаемся с cp1251
        const buffer = readFileSync(filePath);
        return iconv.decode(buffer, 'cp1251');
      } catch (error3) {
        // В крайнем случае читаем как latin1
...
async function importFromCSV() {
  try {
    console.log('🚀 Начинаем полный импорт из CSV файла...');
    ...
    for (let i = 1; i < records.length; i++) {
      ...
      try {
        ...
        if (row.length >= 6) {
          // Вариант 1: imageUrl, name, sku, price, currency, availability, category...
          imageUrl = cleanText(row[0]);
          productName = cleanText(row[1]);
          sku = cleanText(row[2]);
          priceText = row[3];
          availability = cleanText(row[5]) || cleanText(row[4]);
          description = cleanText(row[10]) || cleanText(row[6]) || '';
        } else if (row.length >= 4) {
          // Вариант 2: sku, name, price, description
          sku = cleanText(row[0]);
          productName = cleanText(row[1]);
          priceText = row[2];
...
console.log('📊 ПОЛНЫЙ ИМПОРТ КАТАЛОГА ИЗ EXCEL/CSV');
console.log('='.repeat(60));

importFromCSV()
  .then(() => {
    console.log('\n✅ Полный импорт завершен успешно!');
